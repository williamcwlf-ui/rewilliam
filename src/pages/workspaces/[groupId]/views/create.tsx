...
export default function WorkspacePage() {
  ...
  const createMutation = trpc.workspaces.workspace.views.create.useMutation();

  const [filters, setFilters] = useState<Filter[]>([]);
  const [sorts, setSorts] = useState<Sort[]>([]);
  const [usedDepartments, setDepartments] = useState([]);
  const [color, setColor] = useState<BrandColors>(workspace?.brandColor || 'blue');
  ...
  return (
    <>
      <PageHeading
        title={`${workspace?.groupName} - Create View`}
        disableDivide={false}
      />
      ...
      <form onSubmit={async (submit) => {
        ...
        const selectedDepartments = usedDepartments;
        const selectedFilters = filters;
        const selectedSorts = sorts;

        if (selectedFilters.find(filter => Number.isNaN(filter.value))) {
        ...
        toast.promise(action, {
          ...
          success: 'Successfully created view',
        ...
        <TextLabel label='Name' id="name" placeholder="Best Performers" />
        ...
        <div className='mt-2'>
          ...
          <MultiSelectDropdown onSelectChoice={setDepartments} choices={departments ? departments.map(dep => ({
            name: dep.name,
            id: dep._id.toString(),
          })) : []}
        ...
        <div className='mt-2 border-t border-gray-200 dark:border-gray-700'>
          ...
          <div className='flex flex-col gap-2 mt-2'>
            {filters.map(filter => (
              <div key={filter.id} className='border border-gray-200 dark:border-gray-700 dark:bg-gray-800 px-3 py-3 rounded-xl flex flex-row gap-2'>
                <Dropdown
                  choices={[
                    { id: 'totalMinutes', name: 'Total Minutes' },
                    { id: 'activeMinutes', name: 'Active Minutes' },
                ...
                />
                <Button className="w-min" variant="danger" icon={TrashIcon} onClick={() => {
                  const newFilters = [...filters];
                  const index = newFilters.findIndex(f => f.id === filter.id);
            ...
            <EmptyCardCreateState icon={PlusIcon} cbFunction={() => {
              setFilters([...filters, {
                id: v4(),
                name: 'totalMinutes',
                type: '<',
            ...
            }} buttonText={'Create Filter'} className='w-full' />
          </div>
        </div>
        <div className='mt-2 border-t border-gray-200 dark:border-gray-700'>
          ...
          <div className='flex flex-col gap-2 mt-2'>
            {sorts.map(sort => (
              <div key={sort.id} className='border border-gray-200 dark:border-gray-700 dark:bg-gray-800 px-3 py-3 rounded-xl flex flex-row gap-2'>
                <Dropdown
                  choices={[
                    { id: 'totalMinutes', name: 'Total Minutes' },
                    { id: 'activeMinutes', name: 'Active Minutes' },
            ...
            <EmptyCardCreateState cbFunction={() => {
              setSorts([...sorts, {
                id: v4(),
                name: 'totalMinutes',
                type: 'ASC',
              }])
            }} icon={PlusIcon} buttonText={'Create Sort'} className='w-full' />
          </div>
        </div>
        ...
        <div className='mt-2 border-t border-gray-200 dark:border-gray-700'>
          ...
          <div className='flex flex-col gap-2 mt-2'>
            {computedColumns.map((cc, idx) => {
              ...
              return (
                <div key={cc.key} className='border border-gray-200 dark:border-gray-700 dark:bg-gray-800 px-3 py-3 rounded-xl flex flex-row flex-wrap items-center gap-2'>
                  <TextLabel
                    ...
                    onChange={(val) => {
                      ...
                      next[idx] = { ...next[idx]!, label: String(val) };
                  ...
                  <TextLabel
                    ...
                    onChange={(val) => {
                      const next = [...computedColumns];
                      next[idx] = { ...next[idx]!, expression: String(val) };
                      setComputedColumns(next);
                    }}
                  />
                  <Dropdown
            ...
            <EmptyCardCreateState icon={PlusIcon} cbFunction={() => {
              setComputedColumns([...computedColumns, { key: v4(), label: 'New Column', expression: 'active / total * 100', format: 'number' }]);
            }} buttonText={'Create Computed Column'} className='w-full' />
          </div>
        </div>

        <div className='mt-2 border-t border-gray-200 dark:border-gray-700'>
          <p className='mt-3 text-sm font-medium text-gray-500'>Conditional Formatting</p>
          <p className='text-xs text-gray-400 mb-2'>Highlight cells when a column meets a condition.</p>
          <div className='flex flex-col gap-2 mt-2'>
            {conditionalRules.map((rule, idx) => (
              <div key={rule.id} className='border border-gray-200 dark:border-gray-700 dark:bg-gray-800 px-3 py-3 rounded-xl flex flex-row flex-wrap items-center gap-2'>
                <Dropdown
                  choices={CATEGORY_CHOICES}
                  defaultValue={rule.column}
                  onChange={(val) => {
                    const next = [...conditionalRules];
                    next[idx] = { ...next[idx]!, column: val };
                ...
                <TextLabel
                  ...
                  onChange={(val) => {
                    const next = [...conditionalRules];
                    next[idx] = { ...next[idx]!, value: Number(val) };
                    setConditionalRules(next);
                  }}
                />
            ...
            <EmptyCardCreateState icon={PlusIcon} cbFunction={() => {
              setConditionalRules([...conditionalRules, { id: v4(), column: 'totalMinutes', type: '<', value: 60, color: 'red' }]);
            }} buttonText={'Create Rule'} className='w-full' />
          </div>
        </div>

        <Button type="submit" variant="primary" className='w-full mt-2'>Create View</Button>
